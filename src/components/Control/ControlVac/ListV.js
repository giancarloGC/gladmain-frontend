    ...
    const { documento } = props;
     ...
     useEffect(() => {
         (async () => {
             ...
             const response = await getContVaccApi(documento, token);
             setLoading(false);
             setVacApi(response);
         })();
     }, []);
     ...
     return(
         <Container>
            ...
            {vacApi.length > 0 ? (
             <ListGroup>
             {vacApi.map((item, index) => (
             <ListGroup.Item className="shadow border mt-2 mb-3">
             <Container>
             <Row>
                 <Col md={2} className="align-self-center">
                     <p style={{"color": "#2D61A4", "font-size": 22}}><b>Número Documento </b> <br/> {item.idUsuario} </p>
                 </Col>
                 <Col md={2} className="align-self-center">
                 ...
                 <Col md={2} className="align-self-center">
                     <p style={{"color": "#2D61A4", "font-size": 22}}><b>Vacuna</b> <br/>{item.vacunasRegistradas.length > 0 ? (
                 ...
                 <Col md={2} className="align-self-center">
                     <p style={{"color": "#2D61A4", "font-size": 22}}><b>Fecha Aplicación</b> <br/>{formatedDate(item.fechaAplicacion)}</p>
                 </Col>
