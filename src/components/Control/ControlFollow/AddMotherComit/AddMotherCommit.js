...
export default function AddMotherCommit(props) {
  const { edit, showCommit, setShowCommit, setDataCommit, dataCommit, setSaveData } = props;
        ...
        <Container >
            <Row>
                ...
                onSubmit={(valores, {resetForm}) => {
                  const formData={
                    id: 1,
                    idSeguimientoSalud: parseInt(dataCommit.idSeg),
                  ...
                  insertCompApi(formData, token).then(response => {
                    ...
                    if(response === true){
                      setDataCommit({name: "titulooo", description: "ejemplo description"});
                      setSaveData(false); //PASARLO A FALSE
                      swal({
