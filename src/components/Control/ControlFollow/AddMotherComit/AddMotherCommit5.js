...
export default function AddMotherCommit5(props) {
  const { edit, showCommit5, setShowCommit5, setDataCommit5, dataCommit5, setSaveData5} = props;
  const token = localStorage.getItem(TOKEN);
  
  return(
      <Modal show={showCommit5 ? true : false} size="xs"  onHide={() => setShowCommit5(false)}  centered aria-labelledby="example-custom-modal-styling-title">
        ...
        <Container >
            <Row>
                ...
                initialValues={{ 
                  ...
                  name: "Cuenta con suministro de micronutrientes Hierro, Ácido fólico y calcio y los consume", 
                  description: "",  
                  dateEnd: "",
                ...
                onSubmit={(valores, {resetForm}) => {
                  const formData={
                    id: 1,
                    idSeguimientoSalud: parseInt(dataCommit5.idSeg),
                  ...
                  insertCompApi(formData, token).then(response => {
                    ...
                    if(response === true){
                      setDataCommit5({name: "titulooo", description: "ejemplo description"});
                      setSaveData5(false); //PASARLO A FALSE
                      swal({
                        title: `¡El compromiso fue almacenado correctamente!`,
                        icon: 'success'
                      }).then((value) => {
                        setShowCommit5(false);
                      });
