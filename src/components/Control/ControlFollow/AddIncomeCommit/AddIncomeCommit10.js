...
export default function AddIncomeCommit10(props) {
  const { edit, setShowSuplemento, showCommit10, setShowCommit10, setDataCommit10, dataCommit10, setSaveData10} = props;
  const token = localStorage.getItem(TOKEN);

  const closemodal = () => {
    setShowCommit10(false);
    setShowSuplemento(true);
  }
    return(
      <Modal show={showCommit10 ? true : false} size="xs"  onHide={() => closemodal()}  centered aria-labelledby="example-custom-modal-styling-title">
        ...
        <Container >
            <Row>
                ...
                initialValues={{ 
                  ...
                  name: "La niña o niños recibe suplementación (vitamina A, Zinc, Hierro)", 
                  description: "",  
                  dateEnd: ""
                }}
                validate={(valores) => {
                ...
                onSubmit={(valores, {resetForm}) => {
                  const formData={
                    id: 1,
                    idSeguimientoSalud: parseInt(dataCommit10.idSeg),
                  ...
                  insertCompApi(formData, token).then(response => {
                    if(response === true){
                      ...
                      setSaveData10(false); //PASARLO A FALSE
                      swal({
